...
import { orphanagesData, getOrphanageById } from '@/data/orphanages'
...
function OrphanageDetails() {
  ...
  const orphanage = getOrphanageById(parseInt(id))

  useEffect(() => {
    if (orphanage) {
      document.title = `${orphanage.name} - JALAI`
    }
  }, [orphanage])
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 flex items-center justify-center">
        <Card className="w-full max-w-md shadow-xl border border-white/50 bg-white/90 backdrop-blur-md rounded-3xl">
          <CardContent className="p-12 text-center space-y-4">
            <h2 className="text-2xl font-semibold text-green-800">Orphanage Not Found</h2>
            <p className="text-green-600">The orphanage you're looking for doesn't exist.</p>
            <Button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="shadow-xl border border-white/50 bg-white/90 backdrop-blur-md rounded-3xl overflow-hidden">
              <div className="relative">
                ...
                <div className="absolute top-6 right-6 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center gap-1">
                  ...
                  <span className="text-sm font-medium text-green-800">{orphanage.rating || 5.0}</span>
                </div>
              </div>
            </Card>
            ...
            <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl">
              ...
              <CardContent>
                <p className="text-green-700 leading-relaxed">{orphanage.description}</p>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg border border-white/50 bg-white/90 backdrop-blur-md rounded-2xl">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-green-800">{orphanage.children_count}</span>
                </div>
                
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4 text-green-500" />
                    <span className="text-green-700">Established</span>
                  </div>
                  <span className="font-semibold text-green-800">{orphanage.established}</span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                {orphanage.contact?.phone && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-green-700">{orphanage.contact.phone}</span>
                  </div>
                )}
                ...
                {orphanage.contact?.email && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-green-700">{orphanage.contact.email}</span>
                  </div>
                )}
